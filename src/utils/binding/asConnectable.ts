...
type Connectablized<T extends object> = Pick<Connectable, 'connect' | 'disconnect'> & {
  readonly [K in keyof T]: T[K] extends Subscribable<infer TInner> ? TInner : T[K];
};
  ...
  return new Proxy<Connectablized<T>>(object as Connectablized<T>, {
    get(target, prop, receiver: unknown) {
      if (Object.hasOwn(controller, prop)) {
        return controller[prop as keyof Connectable] as unknown;
      }
